...
\subsection{PP reflection for $V_p$}
...
\subsection{PS reflection for $V_s$}
...
{\bf{\uppercase\expandafter{\romannumeral2}}}:
After the
...
{\bf\uppercase\expandafter{\romannumeral1}} can provide correct traveltime residuals. 
...
Since the PS reflection is asymmetric, the ray path of PS is shown in figure \ref{fig:PS_refl}.
...
\Rome{1} (black) oscillates around the true value (red) up and down. 
...
inversion increases obviously due to this sensitivity. However, if we use method \Rome{2} (blue),
the predicted curves are all below the true value, whose traveltime residual are consistent
with the S-wave velocity error.
If the depth of reflector decreases, the sensitivity in method \Rome{1} reduces (figure \ref{fig:Sens_vp}b). But the
...
According to the above analysis, we recommend method \Rome{2}.
