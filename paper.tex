...
\lefthead{Wang et al.}
\righthead{Elastic WERTI based on mode decomposition}
\maketitle
...
%Therefore, 
Building initial P and S wave velocity models for EFWI by using elastic wave-equation
reflections traveltime inversion (ERTI) would be effective and robust
since taveltime information relates to the background model more linearly. 
The reflection kernels in elastic media are complicated and difficult to use, especially when
...
Besides, P/S separation of multicomponent seismograms can isolate the P or S recordings for
individual data residuals.
%Besides, the model regularization through local dip-dependent smooth filter ensures the inversion converging to a
%geological model. 
Thus, we implement the ERTI by using the $L_2$ norm of the traveltime
residual extracted by the Dynamic image warping (DIW) as objective function.
A two-step inversion strategy is adopted to effectively reduce the nonlinearty of inversion, in which PP reflections are first used to invert $V_p$,
